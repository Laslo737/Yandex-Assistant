...
`YANDEX_MESSENGER_LOGIN_DOMAIN` задаёт домен организации для сопоставления
`user@domain` из Messenger с коротким login Tracker;
если задан, проверяются секретные заголовки. Ответы по задачам доступны только в
личном чате. Анализ отдельных задач и агрегированные сценарии проверяют права
пользователя через Tracker; по решению для MVP кнопка «Мой день» не проверяет ACL
каждой задачи, но требует точного совпадения Tracker ID исполнителя. В анализе
отдельной задачи задачи с компонентами на текущем этапе отклоняются. Проверки на реальных аккаунтах
