...
import { TrackerChangelogEntry, TrackerComment } from '../../../integrations/yandex-tracker/tracker.types';
import { normalizeTrackerIssueBundle } from './tracker-normalizer';
import { TrackerSyncModelDraft, TrackerIssueSyncBundle } from '../domain/tracker-sync.types';

async function fetchAllIssueComments(trackerClient: TrackerApiClient, issueIdOrKey: string): Promise<TrackerComment[]> {
  const perPage = 100;
...
async function fetchAllIssueChangelog(trackerClient: TrackerApiClient, issueIdOrKey: string): Promise<TrackerChangelogEntry[]> {
  const perPage = 100;
  const entriesById = new Map<string, TrackerChangelogEntry>();
  let page = 1;
  let totalPages = 1;
...
export class TrackerSyncService {
  ...
  getStatus() {
    return {
      enabled: env.tracker.enabled,
      configured: this.trackerClient.isConfigured(),
      apiBaseUrl: env.tracker.apiBaseUrl,
      organizationConfigured: Boolean(env.tracker.orgId || env.tracker.cloudOrgId)
    };
  }

  getSetupChecklist(): string[] {
    return [
  ...
  getSyncModelDraft(): TrackerSyncModelDraft {
    return {
      scope: [
        {
          entity: 'issues',
  ...
  async fetchIssueBundle(issueIdOrKey: string): Promise<TrackerIssueSyncBundle> {
    if (!env.tracker.enabled) {
  ...
  async getIssueBundlePreview(issueIdOrKey: string) {
    const bundle = await this.fetchIssueBundle(issueIdOrKey);
    const normalized = normalizeTrackerIssueBundle(bundle);
  ...
  async getIssueBundleDebug(issueIdOrKey: string) {
    const bundle = await this.fetchIssueBundle(issueIdOrKey);
