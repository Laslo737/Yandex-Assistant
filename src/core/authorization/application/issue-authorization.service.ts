...
export type AuthorizationDecision =
  | { allowed: true; reason: 'QUEUE_READ' | 'QUEUE_WRITE' | 'ISSUE_ROLE' }
...
function hasSubject(value: TrackerPermissionSubjects | undefined, fields: Array<'users' | 'groups' | 'roles'>): boolean {
...
export function evaluateIssueAccess(
  issue: TrackerIssue,
  userId: string,
  userPermissions: TrackerQueueUserPermissions,
  queuePermissions?: TrackerQueuePermissions
): AuthorizationDecision {
  if (!issue || !issue.key || !issue.queue?.key || !Array.isArray(issue.components)) {
    return { allowed: false, reason: 'AUTH_CHECK_FAILED' };
  }
  if (issue.components.length) return { allowed: false, reason: 'COMPONENT_UNSUPPORTED' };
  ...
  try {
    ...
    if (read) return { allowed: true, reason: 'QUEUE_READ' };
    if (write) return { allowed: true, reason: 'QUEUE_WRITE' };

    const issueRoles = new Set<string>();
    if (issue.createdBy !== undefined && userIds([issue.createdBy]).includes(userId)) issueRoles.add('author');
    if (issue.assignee !== undefined && userIds([issue.assignee]).includes(userId)) issueRoles.add('assignee');
    if (userIds(issue.followers).includes(userId)) issueRoles.add('follower');
    ...
    if (!issueRoles.size) return { allowed: false, reason: 'NO_PERMISSION' };
    if (!queuePermissions || typeof queuePermissions !== 'object') return { allowed: false, reason: 'AUTH_CHECK_FAILED' };
...
export class IssueAuthorizationService {
  ...
  async filterReadableIssues(userId: string, issues: TrackerIssue[]): Promise<TrackerIssue[]> {
    const result: TrackerIssue[] = [];
    const permissionCache = {
      users: new Map<string, Promise<TrackerQueueUserPermissions>>(),
      queues: new Map<string, Promise<TrackerQueuePermissions>>()
    };
    ...
    await Promise.all(Array.from({ length: Math.min(5, issues.length) }, async () => {
      while (cursor < issues.length) {
        ...
        if (decision.reason === 'AUTH_CHECK_FAILED') throw new Error('Не удалось проверить доступ ко всем задачам. Попробуйте позже.');
        if (decision.allowed) result[index] = issue;
      }
    }));
  ...
  async canReadIssue(
    ...
    issueKey: string,
    cache?: {
      users: Map<string, Promise<TrackerQueueUserPermissions>>;
      queues: Map<string, Promise<TrackerQueuePermissions>>;
    }
  ): Promise<IssueAccessResult> {
    const result = (decision: AuthorizationDecision, queue?: string): IssueAccessResult => {
      const access = { ...decision, issueKey, userId, queue };
      console.info('tracker.issue_access', JSON.stringify(access));
      return access;
    };
    if (!userId || !issueKey) return result({ allowed: false, reason: 'AUTH_CHECK_FAILED' });
    ...
    if (issue.components.length) return result({ allowed: false, reason: 'COMPONENT_UNSUPPORTED' }, queue);
    try {
      ...
      if (direct.reason !== 'AUTH_CHECK_FAILED') return result(direct, queue);
      // AUTH_CHECK_FAILED can mean missing role ACL or malformed response. Do not grant on malformed input.
      if (!userPermissions?.permissions || userPermissions.user?.id !== userId) return result(direct, queue);
