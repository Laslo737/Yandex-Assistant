...
export interface TeamHealthResult {
  manager?: Awaited<ReturnType<ManagerSummaryService['getSummaryByLogin']>>['manager'];
  ...
  topTasks: Awaited<ReturnType<ManagerSummaryService['getSummaryByLogin']>>['topTasks'];
}
...
function getStatusLabel(score: number): string {
  ...
  if (score >= 65) return '🟡 требует внимания';
  return '🔴 зона риска';
}

function getBaseScoreByProblemRatio(problemRatio: number): number {
  if (problemRatio <= 0) return 100;
  if (problemRatio <= 0.1) return 92;
  if (problemRatio <= 0.25) return 82;
  if (problemRatio <= 0.4) return 68;
  if (problemRatio <= 0.6) return 52;
  if (problemRatio <= 0.75) return 38;
  if (problemRatio <= 0.9) return 22;
  return 10;
}

function getQueueHealthLabel(queue: { active: number; overdue: number; stale: number; waitingForReply: number }): string {
  const base = 100
    - queue.overdue * 12
