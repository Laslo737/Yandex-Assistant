...
import { IssueExplainResult } from './issue-explain.service';
...
function buildSystemPrompt(): string {
  return [
    ...
    'Если самый свежий комментарий содержит обещание автора выполнить действие («сейчас запрошу», «уточню», «сообщу»), считай следующий шаг действием этого автора, а не автора предыдущего вопроса.',
...
function buildUserPrompt(result: IssueExplainResult): string {
  return [
...
export class IssueExplainAiService {
  private readonly cache = new Map<string, { expiresAt: number; value: string }>();
  ...
  async generateSummary(result: IssueExplainResult): Promise<string> {
