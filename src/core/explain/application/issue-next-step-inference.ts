...
export interface IssueNextStepInferenceResult {
  lastCommentIntent: IssueCommentIntent;
  lastCommentAuthor?: string;
  lastCommentAt?: string;
  lastCommentSnippet?: string;
  semanticCommentAuthor?: string;
  semanticCommentAt?: string;
  semanticCommentSnippet?: string;
...
function normalizeSnippet(value?: string, maxLength = 220): string | undefined {
...
function findSentence(text: string | undefined, patterns: RegExp[]): string | undefined {
...
function isMeaningfulComment(comment: TrackerComment): boolean {
  ...
  if (/^(ок|окей|хорошо|спасибо|понял|поняла|принято|привет)$/i.test(text)) return false;
  return true;
}

function sortCommentsNewestFirst(comments: TrackerComment[]): TrackerComment[] {
  return comments
    .slice()
    .sort((a, b) => (toDate(b.updatedAt || b.createdAt)?.getTime() || 0) - (toDate(a.updatedAt || a.createdAt)?.getTime() || 0));
}

function isAdministrativeComment(text?: string): boolean {
...
function isSemanticComment(comment: TrackerComment): boolean {
  const text = normalizeSnippet(comment.text, 320);
  if (!isMeaningfulComment(comment)) return false;
  if (!text) return false;
  return !isAdministrativeComment(text);
}

function findLastMeaningfulComment(comments: TrackerComment[]): TrackerComment | undefined {
...
function findLastSemanticComment(comments: TrackerComment[]): TrackerComment | undefined {
  const sorted = sortCommentsNewestFirst(comments);
  return sorted.find(isSemanticComment) || findLastMeaningfulComment(sorted);
}

function classifyIntent(text?: string): IssueCommentIntent {
  const normalized = String(text || '').toLowerCase();
...
function extractWaitingQuestion(comments: TrackerComment[]): string | undefined {
  const sorted = sortCommentsNewestFirst(comments);
...
function humanizeWaitingSubject(text?: string): string | undefined {
  const sentence = findSentence(text, [/жду|ждем|ждём|буду ждать|ожидаем|согласова|ответ|ос|правк|решени/i]);
  if (!sentence) return undefined;

  if (/согласова/i.test(sentence)) return 'согласование / подтверждение решения';
  if (/правк/i.test(sentence)) return 'правки по решению';
  if (/ос|обратн(ую|ой)? связь/i.test(sentence)) return 'обратную связь по текущему варианту';
  if (/решени/i.test(sentence)) return 'решение по следующему шагу';
  if (/ответ/i.test(sentence)) return 'ответ на последний вопрос по задаче';

  return sentence;
}

function humanizeBlockingReason(text?: string): string | undefined {
  ...
  if (/согласова/i.test(sentence)) return 'Задача не двигается без согласования или подтверждения решения.';
  if (/требуется доработка/i.test(sentence)) return 'Текущий вариант требует доработки, поэтому задача не может перейти дальше.';
  if (/нет доступа/i.test(sentence)) return 'Продвижение остановилось из-за отсутствия нужного доступа.';
  if (/ошибк|невозможно|не получается|не можем/i.test(sentence)) return 'Текущий вариант уперся в техническое ограничение или проблему реализации.';
...
function humanizeRequestedAction(text?: string, intent?: IssueCommentIntent): string | undefined {
  ...
  if (intent === 'decision-ready') return 'принять решение или согласовать следующий шаг';
  if (intent === 'blocked') return 'снять блокер и зафиксировать рабочий вариант';
