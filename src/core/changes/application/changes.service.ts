...
export interface ChangesTaskItem {
  key: string;
  summary?: string;
  queue?: string;
  assignee?: string;
...
function isWithinHours(value: string | undefined, hours: number): boolean {
...
function extractDisplay(value: unknown): string | undefined {
  ...
  if (typeof value === 'object') {
    const typed = value as { display?: string; key?: string; id?: string | number };
    return typed.display || typed.key || (typed.id !== undefined ? String(typed.id) : undefined);
  }
  return undefined;
}

function getStatusChangeText(entry: TrackerChangelogEntry): string | undefined {
  ...
  if (to) return `Статус изменен на ${to}`;
...
function sanitizeCommentPreview(text?: string): string | undefined {
  const normalized = normalizePreviewText(text || '');
  if (!normalized) return undefined;
  return normalizeTrackerCommentText(normalized, 90);
}

function extractCommentPreview(comment?: TrackerComment): string | undefined {
...
function findLatestCommentWithinPeriod(comments: TrackerComment[], periodHours: number): TrackerComment | undefined {
  const recentComments = comments.filter((comment) => isWithinHours(comment.updatedAt || comment.createdAt, periodHours));
  if (!recentComments.length) return undefined;
...
function findLatestCommentFromChangelog(
  changelogEntries: TrackerChangelogEntry[],
  comments: TrackerComment[],
  periodHours: number
): TrackerComment | undefined {
...
async function fetchAllIssueComments(
  ...
  const commentsById = new Map<string, TrackerComment>();
  const perPage = 100;
  let page = 1;
  let totalPages = 1;
...
async function fetchAllIssuesByAssignee(
  ...
  do {
    const result = await trackerClient.searchIssues(
      { filter: { assignee }, order: '-updatedAt' },
      { fields, perPage, page }
    );

    for (const issue of result.issues) issuesById.set(issue.id, issue);
    totalPages = Math.max(result.pagination.totalPages || 1, 1);
    page += 1;
  } while (page <= totalPages);
...
async function fetchAllIssuesByQueue(
  ...
  while (true) {
    const result = await trackerClient.searchIssues(
      { queue: queueKey, order: '-updatedAt' },
      { fields, perPage, id: cursorId }
    );
...
export class ChangesService {
  ...
  private async getIssueSignals(issue: TrackerIssue, periodHours: number): Promise<{
    ...
    try {
      ...
      const latestComment =
        findLatestCommentFromChangelog(recentEntries, comments, periodHours) ||
