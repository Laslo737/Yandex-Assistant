import { IssueExplainResult } from '../../explain/application/issue-explain.service';
...
export type IssueHelpIntent = 'summary' | 'why-stuck' | 'next-step' | 'what-happened' | 'generic';

export interface IssueHelpContextPack {
  issueKey: string;
  userRequest: string;
...
function normalizeSnippet(value?: string, maxLength = 500): string | undefined {
...
function summarizeChangeField(value: unknown): string | undefined {
  if (value === null || value === undefined) return undefined;
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') return String(value);

  if (Array.isArray(value)) {
    const parts = value.map((item) => summarizeChangeField(item)).filter(Boolean) as string[];
    return parts.length ? parts.join(', ') : undefined;
  }

  if (typeof value === 'object') {
    const item = value as { display?: string; key?: string; id?: string | number };
...
function buildChangeLine(field: { field?: { display?: string; key?: string; id?: string | number }; from: unknown; to: unknown }): string {
  ...
  if (fromLabel || toLabel) return `${fieldLabel}: ${fromLabel || '—'} → ${toLabel || '—'}`;
  return fieldLabel;
}


function extractRelevantSentence(text: string, patterns: RegExp[]): string | undefined {
...
function findCommentByPattern(bundle: TrackerIssueSyncBundle, patterns: RegExp[]): string | undefined {
  const comments = bundle.comments
    .slice()
    .sort((a, b) => new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime());

  for (const comment of comments) {
    const text = normalizeSnippet(normalizeTrackerCommentText(comment.text, 300), 300);
    if (!text) continue;
...
function findWaitingTopic(bundle: TrackerIssueSyncBundle): { topic?: string; sourceComment?: string; lastConcreteAsk?: string; agreedWhat?: string; remainingOpenPoint?: string } {
  const comments = bundle.comments
    .slice()
    .sort((a, b) => new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime());

  let topic: string | undefined;
  let sourceComment: string | undefined;
  let lastConcreteAsk: string | undefined;
  let agreedWhat: string | undefined;
  let remainingOpenPoint: string | undefined;

  for (const comment of comments) {
    const text = normalizeSnippet(normalizeTrackerCommentText(comment.text, 420), 420);
    if (!text) continue;
...
function buildIssuePurpose(bundle: TrackerIssueSyncBundle): string | undefined {
  const summary = normalizeSnippet(bundle.issue.summary, 220);
  const description = normalizeSnippet(typeof bundle.issue.description === 'string' ? normalizeTrackerCommentText(bundle.issue.description, 420) : undefined, 420);
...
function buildEvidence(input: {
  ...
  if (input.explain.commentSilenceDays !== undefined) evidence.push({ label: 'Комментарии', detail: `Свежих комментариев не было ${input.explain.commentSilenceDays} дн.` });
  if (input.explain.statusSilenceDays !== undefined) evidence.push({ label: 'Статус', detail: `Статус не менялся ${input.explain.statusSilenceDays} дн.` });
  if (input.latestStatusChange) evidence.push({ label: 'Последняя смена статуса', detail: input.latestStatusChange });
  if (input.latestFieldChange) evidence.push({ label: 'Последнее изменение поля', detail: input.latestFieldChange });
  if (input.explain.blockingReasonHumanized) evidence.push({ label: 'Человеческое объяснение блокера', detail: input.explain.blockingReasonHumanized });
  if (input.explain.requestedActionHumanized) evidence.push({ label: 'Какого действия ждут', detail: input.explain.requestedActionHumanized });
  if (input.latestBlockingComment) evidence.push({ label: 'Комментарий с блокером', detail: input.latestBlockingComment });
  if (input.latestAskComment) evidence.push({ label: 'Последний явный вопрос', detail: input.latestAskComment });
  if (input.latestDecisionComment) evidence.push({ label: 'Последнее решение/договоренность', detail: input.latestDecisionComment });
  if (input.latestComment) evidence.push({ label: 'Последний комментарий', detail: input.latestComment });

  return evidence.slice(0, 10);
}

function buildLatestStatusChange(bundle: TrackerIssueSyncBundle): string | undefined {
...
function buildLatestFieldChange(bundle: TrackerIssueSyncBundle): string | undefined {
...
export function buildIssueContextPack(input: {
  issueKey: string;
  userRequest: string;
  intent: IssueHelpIntent;
  bundle: TrackerIssueSyncBundle;
  ...
  const latestComment = normalizeSnippet(normalizeTrackerCommentText(bundle.comments
    .slice()
  ...
  return {
    ...
    meta: {
      ...
      tags: Array.isArray(bundle.issue.tags) ? bundle.issue.tags.filter((item): item is string => typeof item === 'string').slice(0, 10) : [],
      followers: Array.isArray(bundle.issue.followers)
        ? bundle.issue.followers.map((item) => item.display).filter((item): item is string => Boolean(item)).slice(0, 10)
        : [],
    ...
    comments: bundle.comments
      ...
      .map((comment) => ({
        ...
        text: normalizeSnippet(normalizeTrackerCommentText(comment.text, 500), 500)
      })),
...
export function formatIssueContextPackForLlm(pack: IssueHelpContextPack): string {
  return [
...
export function formatCompactIssueContextPackForLlm(pack: IssueHelpContextPack): string {
