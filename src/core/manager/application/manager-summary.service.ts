...
function scoreTask(task: ManagerTaskItem): number {
  let score = 0;
...
function mapTask(issue: TrackerIssue, doneStatusIds: Set<string>, doneStatusKeys: Set<string>): ManagerTaskItem {
  return {
    key: issue.key,
...
export class ManagerSummaryService {
  ...
  private queuesCache: { expiresAt: number; value: TrackerQueue[] } | null = null;
  ...
  private async getQueues(forceRefresh = false): Promise<TrackerQueue[]> {
    const now = Date.now();
  ...
  private async resolveUserByLogin(login: string): Promise<TrackerDetailedUser> {
    const normalized = login.trim();
    const now = Date.now();
  ...
  async isManagerLogin(login: string, forceRefresh = false): Promise<boolean> {
    const normalized = login.trim();
  ...
  async getManagerContextByLogin(login: string): Promise<{
    manager: TrackerDetailedUser;
    queues: Array<{ key: string; name?: string }>;
    terminalStatusNames: string[];
  }> {
