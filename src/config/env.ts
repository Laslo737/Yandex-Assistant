...
export const env = {
  ...
  app: {
    ...
    publicBaseUrl: get('APP_PUBLIC_BASE_URL', get('PUBLIC_BASE_URL', `http://localhost:${get('PORT', '3000')}`)) as string
  },
  yandexMessenger: {
    enabled: getBoolean('ENABLE_YANDEX_MESSENGER', 'ENABLE_YANDEX'),
    apiBaseUrl: get('YANDEX_API_BASE_URL', 'https://botapi.messenger.yandex.net') as string,
  ...
  tracker: {
    ...
    apiBaseUrl: get('YANDEX_TRACKER_API_BASE_URL', 'https://api.tracker.yandex.net') as string,
    webBaseUrl: get('YANDEX_TRACKER_WEB_BASE_URL', 'https://tracker.yandex.ru') as string,
  ...
  llm: {
    ...
    openrouterBaseUrl: get('OPENROUTER_BASE_URL', 'https://openrouter.ai/api/v1') as string,
