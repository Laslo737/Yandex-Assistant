...
- changelog хорошо отражает:
  - смену статусов
  - смену наблюдателей
  - изменение кастомных полей вроде `pendingReplyFrom`, `messengerId` (`pendingReplyFrom` хранится только как raw-сигнал и не используется для определения ожидания или владельца следующего шага)
